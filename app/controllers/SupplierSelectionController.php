...
class SupplierSelectionController
{
    ...
    public function storeQuotation(): void
    {
        ...
        if (!$file || ($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            $this->unprocessable('Debe adjuntar una cotización (archivo).');
        }
        ...
        if ($size > 10 * 1024 * 1024) {
            $this->unprocessable('El archivo excede el tamaño máximo permitido (10MB).');
        }
        ...
        if (!move_uploaded_file((string)$file['tmp_name'], $dest)) {
            $this->unprocessable('No fue posible guardar el archivo adjunto.');
        }

        try {
            $this->repository->addQuotation((int)$process['id'], [
                ...
                'evidence_file_path' => '/storage/selection_quotes/' . $purchaseRequestId . '/' . $filename,
                'evidence_original_name' => (string)$file['name'],
        ...
        $this->repository->updateProcessStatus((int)$process['id'], ['status' => 'EN_EVALUACION']);
